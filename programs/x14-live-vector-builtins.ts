...
function vectorKernel(res: VectorLayout, ctx: ComputeInvocation): void {
  ...
  output.factoryFloat = v4fFrom2(f2, 3.0, 4.0).add(v4fFrom3(f3From, 4.0))
  ...
  output.factorySigned = v4iFrom2(i2, 3, 4).add(v4iFrom3(i3From, 4))
  ...
  output.factoryUnsigned = v4uFrom2(u2, 3, 4).add(v4uFrom3(u3From, 4))
...
export async function main(): Promise<void> {
  ...
  if (deviceResult === null) { print("FAIL device"); adapterResult.dispose(); gpu.dispose(); return; }
  {
    ...
    const hostOutput = hostLayout.output.get(0);
