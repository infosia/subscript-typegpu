...
- **RN14 — Live programs compare pixels with a host rasterizer.**
  Rev 3, 2026-08-25. `x05-live-triangle` draws one flat-colored
  ...
  A color constant in a pixel-oracle program must keep its product
  with 255 away from a half-integer. The rule covers every color
  the program compares and every color a blend consumes (RN21). An
  exact `.5` product is one failure. A product near a half-integer
  is the same failure, because an implementation computes the
  product in its own pipeline. T19 sets the margin and gates the
  rule.
  The float-to-unorm rounding of a tie is implementation-defined:
  for `0.5`, NVIDIA (Vulkan and D3D12) returns 127 and Apple
  (Metal) returns 128 (measured 2026-08-24, `x17-live-indirect`
  pixel 1,2). `0.6` maps to 153 on both.
...
- **RN21 — Blending.** Rev 2, 2026-08-25. `RenderPipelineSpec`
  ...
  T19 gates the source-color rule.
