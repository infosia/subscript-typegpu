...
The "before" rows time one `async_step`. That step resumes the
coroutine after the resolved future, and it runs the rest of
`measureMapAsync`: `readMappedRange`, `unmap`, the byte compare, and
one `print`. The row therefore bounds the host round trip from above.
It is not the round trip alone. The "after" rows time zero steps, so
the same work runs before the timer starts.

The extra suspension cost the Metal host at most 12.3 us per
`mapAsync` before the fix, and nothing after it. A 60 Hz frame is
16.7 ms, so one call cost at most 0.07 per cent of a frame. The cost
was per async call, and it grew with the number of calls in a frame.
...
root export after `main` (subscript `codegen/src/cemit.rs:1271` at `2a65724`).
...
`lib/webgpu.ts` holds 12 script call sites. They are the adapter and
device requests, four pipeline creations, two error-scope pops, the
buffer map, the submitted-work completion, and two device-lost record
