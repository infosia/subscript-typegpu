...
pub(crate) fn support_module(
    ...
    if !pipelines.is_empty() || !render_pipelines.is_empty() {
        out.push_str("import { BindGroupLayoutSpec, COMPUTE_VISIBILITY, FRAGMENT_VISIBILITY, VERTEX_VISIBILITY, VertexBufferLayoutSpec } from \"./typegpu\";\n\n");
    }
        ...
        for layout in &pipeline.layouts {
            ...
            for binding in &layout.bindings {
                ...
                let visibility = match (vertex, fragment) {
                    ...
                    (false, false) => "VERTEX_VISIBILITY + FRAGMENT_VISIBILITY",
                };
                out.push_str(&format!(
