...
@CStruct class Varyings { position: Vec4f; bad: FixedArray<f32, 2>; constructor(position: Vec4f, bad: FixedArray<f32, 2>) { this.position = position; this.bad = bad; } }
function vert(value: Vertex, ctx: VertexInvocation): Varyings { return new Varyings(new Vec4f(value.position.x, value.position.y, 0.0, 1.0), [1.0, 2.0]); }
