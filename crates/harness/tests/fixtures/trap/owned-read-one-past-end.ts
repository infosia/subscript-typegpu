// program: owned-read-one-past-end
// purpose: prove that Buffer.readOne checks bounds before it creates the staging buffer
// exercises: BF9
// questions: none
// expected-rule: BF9
...
export async function main(): Promise<void> {
  ...
  using buffer: Buffer<u32> = createBuffer<u32>(device, 4, 1, GPUBufferUsage.COPY_SRC, "trap-owned-read-one-range");
  await buffer.readOne(device, 1);
}
