...
3. **Async is futures + ProcessEvents, only.** Every webgpu.h callback
   the facade registers uses `WGPUCallbackMode_AllowProcessEvents` and
   terminates inside the facade. Scripts poll futures around
   `Context.suspend()`. No facade callback ever unwinds or calls back
   into webgpu.h.
...
3. **One test executable per crate.** Integration tests live in
   `tests/main.rs` with one module per topic. Suites are data-driven
   over `programs/`, so a new program adds no Rust code.
